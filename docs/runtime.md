...
`internal/version.Value` starts as `"dev"`. During package initialization, an unchanged `"dev"` value is replaced with the trimmed contents of the compile-time embedded [`internal/version/version`](../internal/version/version) file, currently `0.10.0`. Release builds can replace `internal/version.Value` through Go linker flags; when replaced with a value other than `"dev"`, that linker-provided value is printed instead.
