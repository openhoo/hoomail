...
Matching CID image sources are rewritten before final HTML sanitization. Resolved CID SVG resources additionally pass through a strict XML-token allowlist before inline delivery: malformed XML fails closed, active/animation/foreign elements and style/event attributes are removed, and only internal fragment references may survive. The original SVG attachment remains download-only. Safe absolute `http:`, `https:`, and `mailto:` links remain sanitized links with `target="_blank"` and `rel="noopener noreferrer"`; the preview iframe permits them to open a new tab with only `allow-popups` and `allow-popups-to-escape-sandbox`. The opened page runs outside the email sandbox and receives neither `window.opener` nor a referrer. Use Inspect to review destinations before opening them separately. Neither inspection nor preview fetches remote image URLs.
