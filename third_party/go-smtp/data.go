...
func newDataReader(c *Conn) *dataReader {
	...
	if c.server.MaxMessageBytes > 0 {
		dr.limited = true
		// One-byte sentinel: the counter must stay positive while the
		// end-of-data marker (<CR><LF>.<CR><LF>) following a message of
		// exactly MaxMessageBytes bytes is consumed, otherwise Read would
		// report ErrDataTooLarge before seeing the terminator. Messages
		// larger than MaxMessageBytes still overflow on their next read,
		// matching BDAT's strict cumulative size check.
		dr.n = int64(c.server.MaxMessageBytes) + 1
	}
